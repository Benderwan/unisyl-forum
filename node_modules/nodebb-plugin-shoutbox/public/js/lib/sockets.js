...
(function(Shoutbox) {
	...
	var Sockets = function(sbInstance) {
		...
		this.events = Events;
		this.typing = [];
		this.typingHandler = function(){
			console.log(this.typing);
			if(this.typing.length == 1){
				console.log("1 typing");
				$('.shoutbox-typing').html(this.typing[0].username + " écrit...");
			}else if(this.typing.length > 1){
				console.log("2 or more typing");
				var html = "";
				for(i = 0; i < this.typing.length - 1; i++){
					html = html + this.typing[i].username + ", "
				}
				$('.shoutbox-typing').html(html + " et " + this.typing[this.typing.length - 1].username + " écrivent...");
			}else{
				console.log("None typing");
				$('.shoutbox-typing').html("");
			}
		}.bind(this);
		...
		this.handlers = {
			...
			onStartTyping: function(data) {
				console.log(data);
				var inarray = false;
				for(i = 0; i < this.typing.length; i++){
					if(this.typing[i].uid == data.uid){
						inarray = true;
					}
				}
				if(!inarray){
			...
			onStopTyping: function(data) {
				var remove = data.uid;
				console.log("Removing " + data.uid);
				this.typing = this.typing.map(function(elem){
					if(elem.uid && elem.uid != remove){
						return elem;
					}
				});
				this.typingHandler();
