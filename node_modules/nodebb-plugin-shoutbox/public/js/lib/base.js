...
(function(Shoutbox) {
	...
	Instance.prototype.addShouts = function(shouts) {
		...
		shouts = shouts.map(function(el) {
			...
			el.showTime = el.timestamp - lastTime >= 3600000;

			// Add shout chain information to shout
			el.isChained = lastUid === uid && !el.showTime;
			...
			el.isMine = el.isOwn;
			el.isAdmin = el.user.isAdmin;

			lastUid = uid;
			lastSid = sid;
			lastTime = parseInt(el.timestamp);
		...
		this.vars.lastTime = lastTime;

		templates.parse('shoutbox/shouts', {
			shouts: shouts
		}, function(html) {
			self.dom.shoutsContainer.append(html);
			if(!window.chatScrollbar){
				window.chatScrollbar = new GeminiScrollbar({
				    element: self.dom.shoutsContainer
				}).create();
			}else{
	...
	function setupVars() {
		this.vars = {
			...
			scrollBreakpoint: 50,
			messages: {
				alert: '[ %u ] - Nouveau message!',
				empty: 'Le chat est vide, entrez un message !',
